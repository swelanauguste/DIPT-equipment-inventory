        ...
        try:
            with open(file_path, mode="r", encoding="utf-8") as file:
                ...
                for row in reader:
                    ...
                    slug = slugify(name)
                    if not Maker.objects.filter(slug=slug).exists():
                        Maker.objects.create(name=name, slug=slug)
                        added_count += 1
                        self.stdout.write(f"Added Maker: {name}")
                    else:
                        skipped_count += 1
                        self.stdout.write(f"Skipped existing Maker: {name}")
